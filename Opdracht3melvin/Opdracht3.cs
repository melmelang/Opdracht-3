...
namespace Opdracht3melvin
{
    public partial class Opdracht3 : Form
    {
        ...
        Stapel<Animal> dierStapel = new();
        bool dierVeranderd = false;
        public Opdracht3()
        {
            InitializeComponent();
        }
        ...
        private void toevoegenObject_Click(object sender, EventArgs e)
        {
            ...
            {
                dierStapel.Toevoegen(new Animal(objectNaamBox.Text, int.Parse(objectLeeftijdBox.Text)));
            }
        }
        ...
        {
            if (dierStapel.ToString() != string.Empty)
            {
                dierStapel.Verwijderen();
            }
            else
            {
                MessageBox.Show("Sorry maar de lijst is leeg");
        ...
        {
            MessageBox.Show(dierStapel.ToString());
        }
        ...
        private void isAanwezigObject_Click(object sender, EventArgs e)
        {
                ...
                string.IsNullOrWhiteSpace(objectNaamBox.Text) == false)
            {
                ...
                if (
                    dierStapel.IsAanwezig(new Animal(objectNaamBox.Text, int.Parse(objectLeeftijdBox.Text)))
                    )
                {
        ...
        {
            dierStapel.StapelCopy();
            MessageBox.Show("Old list: " + dierStapel.ToString() + "\n New list: " +
                String.Join("; ", dierStapel.StapelCopy()));
        }
    }
}
